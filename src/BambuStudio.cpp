...
#include "libslic3r/CutSurfaceHelper.hpp"  // cut_surface_helper_main()
...
{
    // Helper-process dispatch -- handle BEFORE any GUI / TBB / wx init.
    // When this binary is re-exec'd by Slic3r::cut_surface_via_helper() with
    // argv[1] == "--cut-surface-helper", it does NOT run the GUI; it reads a
    // serialized cut_surface() request from stdin, runs the call, writes the
    // result to stdout, exits. Keeping this dispatch FIRST means helper
    // launches stay cheap (no wxApp constructor, no plugin load) and means
    // the helper inherits the parent's code signature without any extra
    // entitlement work (it's the same Mach-O binary, just argv differs).
    ...
    // hook only fires on THREAD_START, so it must be installed before the first
    // thread is created to cover the TBB slicing-pool workers that run CGAL/GMP
    // geometry and can overflow their stacks. If armed later, those pre-existing
    // threads get no alt stack and a stack-overflow signal can't be delivered to
    // our handler (observed: raw __gmpz_* crash, no handler frame).
    Slic3r::install_process_wide_altstacks();
#endif
