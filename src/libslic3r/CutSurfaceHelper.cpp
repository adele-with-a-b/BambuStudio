...
int cut_surface_helper_main()
{
    ...
    SurfaceCut cut = Slic3r::cut_surface(shapes, models, projection, projection_ratio);

    // Write response. If cut_surface threw a HardCrash or longjmp'd,
    // we never reach here -- the caller (parent) will see signal-death
    // or non-zero exit and treat that as "failed; return empty cut".
